...
其中Pold代表真正吃了batch的经验的Actor，而P表示ppo_epochs实时迭代更新的Actor，这个公式也可以理解为：在Actor想通过模拟交互的方式，使用一个batch的经验值更新自己时，它需要收到真正吃到batch的那个时刻的Actor的约束，这样才能在有效利用batch，提升训练速度的基础上，保持训练的稳定。另外相比之前的Actor_loss丢弃了log，设计到了重要性采样的内容  
  ...
  重要性采样是一种概率校正手段，可以用一个分布的数据估计另一个分布下的期望，因为我们的经验是最初的那个模型产生的，随着ppo_epochs的进行，这个经验先优化了一次模型，得到新的策略，然而经验中的动作还是符合最初的模型的分布，如果想继续优化下去，则需要改变动作的分布，故引入重要性权重来校准，用于估计当前策略下的目标函数。比如：最初的模型针对一个产生了一个prob，随后更新后得到新的模型并产生新的prob，用这个两个比值作为权重来衡量新模型的目标函数。  
![image](https://github.com/user-attachments/assets/d2929834-18b1-4c9e-9fb6-1c17ddc00cbc)
